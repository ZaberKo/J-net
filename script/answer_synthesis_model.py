...
class AnswerSynthesisModel(object):
    ...
    def question_encoder_factory(self):
        with default_options(initial_state=0.1):
            model = Sequential([
                Embedding(self.emb_dim, init=self.npglove_matrix),
                Stabilizer(),
                # ht = BiGRU(ht−1, etq)
    ...
    def model_factory(self):
        ...
        emb_layer = Embedding(self.emb_dim, init=self.npglove_matrix)
    ...
    def model_train_factory(self):
        ...
        def model_train(question, passage, answer):
            past_answer = Delay(initial_state=self.start_word)(answer)
    ...
    def criterion_factory(self):
        @C.Function
        def criterion(answer: self.AnswerSequence, answer_label):
    ...
    def model(self):
        question_seq = C.sequence.input_variable(self.vocab_dim, sequence_axis=self.question_seq_axis, is_sparse=True,
                                                 name='question')
        passage_seq = C.sequence.input_variable(self.vocab_dim, sequence_axis=self.passage_seq_axis, is_sparse=True,
                                                name='passage')
        answer_seq = C.sequence.input_variable(self.vocab_dim, sequence_axis=self.answer_seq_axis, is_sparse=True,
                                               name='answer')

        model = self.model_train_factory()
        criterion_model = self.criterion_factory()

        synthesis_answer = model(question_seq, passage_seq, answer_seq)
