...
def train(data_path, model_path, log_file, config_file,  isrestore=False, profiling=True, gen_heartbeat=False):
    ...
    lr = C.learning_parameter_schedule( training_config['lr'], minibatch_size=mb_size, epoch_size=epoch_size)
    momentum=C.momentum_schedule(training_config['momentum'],minibatch_size=mb_size,epoch_size=epoch_size)

    learner = C.adam(model.parameters,lr,momentum)
